...
(function () {
  ...
  // magenta ramp — keyed to the real gem.png currency (deep facet → gem core → bright)
  const AMBER = [
    null,
    { c: [150, 28, 140], a: 0.24 },
    { c: [224, 40, 160], a: 0.46 },
    { c: [255, 77, 205], a: 0.70 },
    { c: [255, 150, 230],a: 0.88 },
  ];
  const CYAN = [
    null,
    { c: [30, 130, 178],  a: 0.24 },
  ...
  function init(canvas, cfg) {
    ...
    // ── external control: tint 0..1 (cyan→amber) + intensity scale ──
    let tint = 0, extScale = 1;
    function tierColor(lvl) {
      const cc = CYAN[lvl].c, ac = AMBER[lvl].c;
      const r = Math.round(cc[0] + (ac[0] - cc[0]) * tint);
      const g = Math.round(cc[1] + (ac[1] - cc[1]) * tint);
      const b = Math.round(cc[2] + (ac[2] - cc[2]) * tint);
      return r + ',' + g + ',' + b + ',' + CYAN[lvl].a;
    }
    ...
    function draw(intensity) {
      ...
      for (let y = 0; y < ROWS; y++) {
        for (let x = 0; x < COLS; x++) {
          ...
          if (lvl <= 0) continue;
          if (lvl > 4) lvl = 4;
          ctx.fillStyle = 'rgba(' + tierColor(lvl) + ')';
    ...
    const handle = {
      set: function (o) {
        if (o && o.tint != null) tint = Math.max(0, Math.min(1, o.tint));
        if (o && o.scale != null) extScale = o.scale;
      },
      reset: function () { tint = 0; extScale = 1; }
    };
    ...
    let t = 0, dipUntil = 0, nextDipRoll = 0, last = 0;
    function loop(now) {
