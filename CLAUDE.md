...
| Companion (BIT) | `BitSprite`/`BitSpeechBubble` (`widgets/companion/`), `bitAddress` (`data/companion_address.dart`) | Mascot **BIT** — a pixel "drone core" (raster sprites in `assets/mascot/bit-sprites/`, painted `errorBuilder` fallback). The system's faceless voice through onboarding; **embodies** at the `StartGateScreen` with the first name-drop ("What should we do first, {name}?"). Subordinate to the user-hero — never on the identity frame. Address register: name (intimate) / "warrior" (ceremony) / "recruit" (pre-embodiment), "warrior" the fallback. Interview voice + in-app presence are planned follow-ups. |
