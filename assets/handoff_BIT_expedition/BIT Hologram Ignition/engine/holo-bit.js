/* ============================================================================
   ...
   Usage:
     ...
     holo.start();   // power-on; pass a delay (ms) for the dark gap: holo.start(2000)
     holo.stop();    // tear down (BIT comes home / relaunches) — also cancels a pending delay
   ============================================================================ */
const HoloBit = (function () {
  ...
  function create(cfg) {
    ...
    let raf = 0, t0 = 0, last = 0, fade = 0, flick = 1, delayTimer = 0;

    // Ignition envelope (ms → 0..1): a struggling-tube STUTTER that catches.
    // Coarse keyframes — the loop is ~20fps, so the stutter stays chunky.
    function igniteEnv(ms){
      if (ms >= 900) return 1;
      const K = [[0,0.0],[50,0.9],[110,0.04],[180,0.62],[250,0.0],[300,0.72],
                 [380,0.08],[440,1.0],[520,0.18],[600,0.92],[700,0.4],[820,1.0],[900,1.0]];
      for (let i=1;i<K.length;i++){
        if (ms <= K[i][0]){ const a=K[i-1], b=K[i], u=(ms-a[0])/(b[0]-a[0]);
    ...
    function frame(now) {
      ...
      last = now;
      const t = (now - t0)/1000;
      const ign = igniteEnv(now - t0);              // stuttering power-on, then steady
      fade = ign;                                   // BIT's opacity follows the ignition
      flick = (0.82 + 0.18*Math.sin(t*7) - (Math.random() < 0.08 ? Math.random()*0.3 : 0))
              * (0.3 + 0.7*ign);                    // rig stutters on with the image
    ...
    return {
      // delayMs: wait that long (dock empty) before igniting. Omit/0 = immediate.
      start(delayMs) {
        if (delayTimer){ clearTimeout(delayTimer); delayTimer = 0; }
        if (RM || !delayMs){ begin(); return; }
        holoCanvas.style.display = 'none';         // empty dock during the gap
        delayTimer = setTimeout(() => { delayTimer = 0; begin(); }, delayMs);
      },
      stop() {
        if (delayTimer){ clearTimeout(delayTimer); delayTimer = 0; }
