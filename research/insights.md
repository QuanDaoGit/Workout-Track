...
### Stat pacing + the ×10 rescale — a RESOLUTION fix, not the whole fix; band-local visible progress is the real requirement (2026-07-22)
Feeds the stat-math re-tune (per-session STR/AGI/END gains collapse to +1/+0 from ~stat 650; A→S ≈ 1,024 sessions). Decision target: [stat_engine.dart](../app/lib/services/stat_engine.dart) `_statFromVolume` curve + rank thresholds + [strength_standards.dart](../app/lib/data/strength_standards.dart) calibration targets + soul (competence/visible deltas). **Codex evidence-review run (prompt-carried `task`, substantive)** — all 8 challenges answered, folded below. Builds on the 2026-06-21 math-system battery ([[research-4-combat-stats]], #3 XP re-curve, #6 calibration, #10 overload).
- `[validated, peer-reviewed — bounded]` **Point magnitude itself motivates** (Medium Maximization, [Hsee et al., JCR 2003](https://academic.oup.com/jcr/article-abstract/30/1/1/1801715): people maximize the point-medium independent of outcome value). Supports "bigger numbers, bigger dopamine" — but per Codex it proves *magnitude matters*, not that ×10 lifts retention; big denominations keep credibility only while increments stay interpretable.
- `[validated, quantitative — the load-bearing find]` **×10 multiplies delta *resolution*, so it is NOT purely cosmetic**: a session paying +1.4 (displayed +1) at old-stat 650 displays +14 at ×10; the invisible-progress wall moves from ~650 to ~the S band. It does NOT fix the A→S span (~1,024 sessions unchanged), the elite calibration dead-start (+0 forever), or the class bonus compressed to +18 lifetime by ln().
- `[validated, contrary — boundary]` **Number-inflation harm is about COMPOUNDING growth + legibility collapse** (WoW's repeated stat squishes, 8-digit damage; [warcraft.wiki](https://warcraft.wiki.gg/wiki/Stat_squish)), not a one-time pre-launch denomination change. Bound: keep stats ≤5 digits, never re-inflate later; radar/chip readability at 4–5 digits is an open `ironbit-design` check (Codex F7).
- `[validated, practitioner + prior battery]` **"Never a wall where progress feels impossible, never a plateau where progress feels meaningless"** — remedies are per-session visible increments, milestone density, band-local pacing, prestige framing. A distant top rank is defensible ONLY with frequent meaningful markers en route (Codex F5: A→S as shown-next-rank at 6.5 years = dead aspiration; as explicitly mythic prestige = fine).
- `[risk, per Codex F3]` **A per-session stat floor is defensible only if gated + bounded**: ride the XP reward-eligibility gate (≥15min ∨ ≥200kg ∨ ≥3 exercises), keep the floor small and capped with the main delta still work-driven — else it hollows the competence signal into a participation trophy ([[research-4-combat-stats]] junk-volume hazard).
- `[validated, per Codex F6]` **Calibration must leave band-local runway**: seeding advanced/elite users AT their tier target gives the most invested lifters the least feedback (advanced +1/session, elite +0 from day one). Pattern: seed *below* the band midpoint / separate "starting credential" from "earned growth" — advanced users need slower growth, never no growth.
- `[risk, per Codex F4+F8]` **"The log curve is domain-honest" is not load-bearing** — the stat is a game competence signal, not dosimetry; believable progression is the requirement, not physiological fidelity. Saturation may stay, *invisible* saturation may not. And the strongest case against ×10 alone: it treats a feedback-system problem as a denomination problem — ship it only alongside band-local pacing + calibration runway.
- **Decision feed:** ×10 rescale = YES as resolution fix (pre-launch, no migration cliff), but paired with: a gated bounded per-eligible-session floor, calibration runway (seed below band midpoint), a re-spanned or explicitly-mythic S, and a visible (non-ln-compressed) class bonus. → `/deep-feature` Stage 3 opinion. Links [[research-4-combat-stats]].
...
### Interaction-tier SFX — 3-tier loudness ladder, core-loop-first, pooled low-latency channel, audio-focus defect (2026-07-18)
Feeds the interaction-SFX feature (deep-feature; user picked "engine + UI tap layer + set-log/rest-end" from an audio audit). Reuses the settled 2026-06-20/21 audio entries (fatigue->variants, square+noise timbre, pitch grammar, sound-not-gated-by-reduced-motion, own mute). **Codex-reviewed twice** (evidence: needs-attention 5 findings; opinion/plan: needs-attention 5 findings — all folded).
- `[validated, primary source]` **audioplayers' Android default AudioContext = AUDIOFOCUS_GAIN + USAGE_MEDIA, and `SfxService` never overrides it — every app sound can pause/duck the user's own music.** Hevy shipped this exact bug (rest sound stopped Spotify; fixed 1.26.11). Fix for ALL SFX channels: audioFocus **none** (mix-with-others) + contentType sonification + usageType assistanceSonification. `[risk, per Codex]` propagation to AudioPool players unverified — needs the on-device Spotify-keeps-playing check at integration.
- `[validated, docs]` **Engine: audioplayers AudioPool + PlayerMode.lowLatency** (Android SoundPool-backed, preloaded players) behind a narrow `SfxService.playUi` seam; flutter_soloud = pre-approved drop-in fallback behind the same seam. `[risk, per Codex]` docs-only — on-device tap-to-ear latency is a HARD GATE before broad rollout.
- `[validated, competitor]` **Hevy ships the whole tier**: a "check set" sound on set completion, a rest-timer sound (5 options), a PR alert — each with off/low/normal/high volume. Rest-end cue + set-log sound = market table stakes; per-class volume control precedented. Duolingo ships UI sounds default-ON with a toggle (secondary).
- `[validated + dissent recorded]` **Restraint doctrine** (Material: secondary sounds = infrequent/functional; "feedback only, less is more"): resolved into **core-loop-first** — set-log/rest-end/warning ship first on the master Sound toggle; TAP ticks behind a dedicated "UI sounds" sub-toggle (default = product-owner call after on-device audition); SELECTION ticks deferred to v2 entirely. Ticks are deliberately redundant-with-haptics under music.
- `[validated, per Codex]` **Context-of-use governs the audio design:** gym users wear earbuds with music/podcasts — mix-with-others means our sounds OVERLAY theirs. Ticks must be sub-music-trivial; the rest-end chime is best-effort in-app (the system notification stays the only reliable alarm); audition/validation must happen OVER music+speech beds.
- `[validated, heuristics]` **Craft (downgraded from GameJuice blog + settled insights):** ticks 16-35ms very quiet; confirms ~100-130ms; all UI sounds <500ms; forward/confirm ascends-or-stays, error/destructive DESCENDS; one musical key across the palette (ours = C major, matching the fanfares); phone-speaker floor — primary semantic cue >=~700 Hz (small speakers kill lows); band-limit synthesis (naive C6+ squares alias); a routine confirm should be NEUTRAL (a rising "collect" figure on every set inflates audio celebration vs the real PR/reward class); loudness ladder authored as peaks: sel 0.07 < tap 0.10 < warn 0.20 < log 0.22 < rest 0.30 < ceremony 0.32.
- **Decision feed:** 15 candidate mocks synthesized (3/role + warning + beds) into an ironbit-themed HTML audition page (phone-speaker sim high-pass, music/voice overlay beds, coalesce-aware rapid-fire, cross-tier session sim, silence baselines) — user picks, then integration: pooled channel + audio-focus fix + wrapper seam (sound BESIDE HapticService, never inside — separate toggles; own coalesce window) + set-log (3 variants + ~1s class cooldown) + rest-end under the existing live-finish guard. FilledButton gap (55 sites outside the wrapper system) inherited from haptics, flagged follow-up. Links the 2026-06-21 haptics entries (the same coverage-vs-intensity doctrine, now on the audio channel).


### SFX v2 — partial sound coverage reads as BROKEN; the fix is a role KIT with structural firing (2026-07-19)
On-device verdict on the v1 interaction-SFX pass: "really weird, half baked, not coherent, a little here a little there" — sparse coverage is WORSE than none (users hear randomness, not restraint). Full interaction inventory (2 exhaustive sweeps) confirmed only PixelButton auto-played; chips/switches/raw-buttons/sheets/world-objects were silent; success-intent buttons read as dead keys. **Codex-reviewed** (kit design: needs-attention, 6 findings folded).
- `[validated, user-tested]` **Coverage coherence beats per-sound restraint.** The Material "secondary sounds should be infrequent" doctrine loses to grammar-completeness once ANY tier ships: silence gaps read as bugs. Resolution = a role kit (select/toggle/stepper/tick/warn/skip/confirm/rest-tiered/notice + world-object signatures) fired STRUCTURALLY (wrapper defaults + drop-in ArcadeFilled/TextButton/IconButton + CI ban on bare Material buttons, empty baseline) so a new surface cannot ship silent.
- `[validated, per Codex]` **One owner per gesture + an arbiter.** Blanket mechanisms (theme splashFactory = phantom pointer-down ticks while scrolling; NavigatorObserver open/close = system dialogs "responding" to nobody) were rejected for semantic triggers; collisions handled by precedence (signature > warn > role > tick) + an 80ms micro-suppression window after any louder role. Audio intent is declared per site, never derived from the haptic intent (tactile ≠ audible — Codex).
- `[validated, user-taste]` **Sound character comes from the component's fiction/animation, not a catalog:** chip blink → digital data-zip (noise transients read as "random clicks" — pure square); quest board → degauss/static texture (tones read "beep beep", not a screen); pad → the dispatch whoosh's DNA (melodic arps read "funny", not futuristic); BIT → 3-syllable speech prosody with a rising question bend (short chirps read "generic" — personality needs length + contour). Iterated via the audition page in 2 rounds before any wiring.
- **Decision feed:** UiSound registry + playUi arbiter in SfxService; rest re-tiered (between-set weak sibling / between-exercise chorus / skip release); toggles+steppers whisper-tier (first cut if density grates on device); bitChirp is a ONE-OFF character voice (scope-guarded). Links the 2026-07-18 entry (engine/audio-focus) + the haptics coverage-vs-intensity doctrine.


### Onboarding SFX ceiling — bespoke EVENT-SFX at the peaks, NO music bed, budgeted density (2026-07-23)
Foundational grammar decision before designing each onboarding beat's sound in its own workflow. Onboarding (~16-beat first-run cinematic) is mostly silent except the tail (charge-ritual reel + gift-reveal already bespoke; a couple chip taps). Fork: (A) bespoke event-SFX only, no bed; (B) full scoring + a music bed; (C) reuse existing kit only. **Codex-reviewed the evidence** (needs-attention → 5 should-fix + 2 nits, all folded).
- `[validated, engineering-fit — load-bearing, the crux]` **A music bed (B) is contraindicated by the app's own `audioFocus=none` doctrine:** the app never ducks the user's music, so a continuous bed would play SIMULTANEOUSLY over their Spotify = two tracks at once. Short EVENT SFX layering over the user's music is fine (notification-blip model); a persistent bed is not. Also the charge-ritual reel is ALREADY a contained musical/voiceover moment, so a bed is partly redundant + would collide with it. ([Android audio focus/ducking](https://developer.android.com/media/optimize/audio-focus))
- `[validated, technical — defeats one objection, not a green light]` **Android silent/ringer mode does NOT mute media-stream app audio** (media is a separate volume stream from ringer/notifications) — so "~46–51% on silent/vibrate" ([Android Authority 2021](https://www.androidauthority.com/sound-profile-vibrate-ring-silent-poll-results-1224669/)) does NOT mean half won't hear onboarding SFX. **Per Codex F3: use this to reject "silent = inaudible," NOT to infer users WANT audio** — sound stays enhancement-not-essential, easily muted.
- `[validated, competitor — directional]` Duolingo treats sound+animation as "a retention investment, not aesthetics"; mascot-driven feedback → emotional loyalty ([userguiding teardown](https://userguiding.com/blog/duolingo-onboarding-ux)) — but uses EVENT sounds + voice, NOT an onboarding music bed. ~60–73% of mobile gamers play with sound ON, only ~9% fully silent ([TouchArcade 2013](https://toucharcade.com/2013/11/05/73-of-mobile-gamers-play-with-the-sound-on/)) → majority hear it, real minority don't. (Poll %s are 2013–2022, US-skewed → directional only, per Codex.)
- `[validated, UX guideline]` Autoplay-without-interaction is disruptive, but onboarding SFX fire on the user's OWN taps + inside a cinematic they chose to enter = post-interaction, NOT cold autoplay. Apple HIG / Microsoft: "don't make users configure their way out of an annoying initial experience," don't sound high-frequency events, provide control ([Apple HIG audio](https://developers.apple.com/design/human-interface-guidelines/patterns/playing-audio/)).
- `[validated, Codex F4 — the density discipline]` **Beat budget: author bespoke SFX ONLY for the ~4–6 narrative PEAKS** (CRT boot power-on, BIT face-reveal surge, class-reveal stamp, name/birth chime, rank stamp); transition/crossfade beats stay quiet or interaction-kit only. Prevents the busy/childish/fatiguing first-run failure. Not every beat gets sound.
- `[validated, Codex F5]` **Mix boundaries:** loudness hierarchy interaction < event < ceremony; NO tonal tails bleeding into the reel or the gift-reveal ceremony; silence/sparse SFX at the reel's edges. `[risk]` preload of many AudioPools at first launch could jank the intro on low-end Android → lazy/deferred pooling.
- **Decision feed → recommendation = A, layered over the kit, no bed for v1:** (1) reuse the kit for functional taps (C insight, taps only — already structural); (2) author a small bespoke onboarding event-SFX register for the peaks (budgeted), co-designed with the already-specified haptic beats, self-generated CC0, varied-on-repeat; (3) **reject B for v1, not forever** — the strongest B (a bounded cinematic-audio mode that requests transient duck for the ~60–90s intro + restores on skip/background) is legitimate, revisit only if user-testing shows uplift (Codex F1); (4) default-ON **but** add an early discoverable mute affordance INSIDE onboarding before audio escalates (Settings isn't reachable at first-run — Codex F2), persisted to master Sound; (5) reject C as the ceiling on design judgment (reuse-only recycles generic ticks on the peaks → the documented "half-baked" failure), not on external proof (Codex nit). Then each beat → its own per-component design→implement workflow off a prioritized backlog. Links the 2026-07-18 (audio-focus) + 2026-07-19 (SFX v2 kit) entries + the 2026-06-23 haptics onboarding-beat map (audio must co-design with those haptic beats).


### BIT flight/land ceremony SFX — a rising pitch-GLIDE reads as a slide-whistle; redesign noise-first (2026-07-23)
The user disliked the shipped `ceremony_flight`/`ceremony_land` ("annoying gut feeling, can't say why"). Root-caused it: the old flight was a **monophonic RISING PITCH-GLIDE** (140→1000 Hz square) + a **pitch-DROP** land — and a rising/falling pitch glissando IS the **slide-whistle COMEDY idiom** (Wikipedia: used for comedic rising/falling gags), which reads cheap/grating even when you can't name it.
- `[validated, craft]` **Whoosh momentum should come from FILTERED NOISE + amplitude arcs, NOT a tonal pitch-slide.** HP the lows (<80–100 Hz), a mid presence bump ~1–3 kHz to cut through, layer a base sweep + a sub for weight + a high air layer ([professionalcomposers](https://professionalcomposers.com/how-to-make-whoosh-sounds/), [krotosaudio](https://sound.krotosaudio.com/whoosh-sound-effects/)). Swoosh = the sharper/more-defined variant.
- `[validated, craft]` **Impacts/lands = transient (crack) + body (weight) + tail (debris)**, transients aligned for punch ([SFX Engine impact guide](https://sfxengine.com/blog/impact-sound-effect)).
- **Decision:** auditioned 5 fresh noise-driven approaches (jet / air-swish / warp / servo / arcade). User picked **flight = V5 chunky-arcade** (noise dash + a DISCRETE bright stab, no slide) + **land = V4 servo-latch** (click + inharmonic metallic ring + power-down thunk — fitting BIT-the-drone-core). Root place = `ops/gen_flight_land_sfx.py` + reusable `SfxService.playFlight()/playLand()` (the ceremony names delegate). Links the 2026-07-23 onboarding-SFX entry above.
- **Reusable takeaway:** when a synthesized "motion" SFX feels cheap/annoying, suspect a **monophonic pitch-glide** first — swap it for filtered-noise-driven momentum.


### Charge-ritual reel → Start Gate — triple verbal stream + admin-first gate (2026-07-23)
On-device novice verdict on the shipped sequence: reel "not cinematic", "too fast", BIT's dialogue clashes ("reading 2 things at once"), clip→gate transition "too fast", Start Gate = "a drop of motivation." Grounded audit (code timing map; 1fps frame extraction of `charge_ritual_reel.mp4`; `test/audit/_shots/start_gate.png`). **Codex-reviewed at design stage** (needs-attention → 1 high + 4 medium, all folded).
- `[validated, grounded]` **The reel is a triple verbal stream.** The 19.9s asset carries burned-in kinetic captions (~1 new 3–6-word fragment per ~1.7s) + spoken coach audio; BIT typewriters two lines DURING playback — the second at video-pos 4s, exactly when the captions begin the message payload — plus a skip link that onsets at 3s and a continuously filling charge bar. Subtitle reading is automatic/near-mandatory (d'Ydewalle & de Bruycker: viewers fixate subtitles regardless of intent) and motion onsets capture saccades → captions and the typing bubble compete for the SAME single foveal channel (split-attention/redundancy, Mayer/Sweller). "The clip is too fast" is substantially STOLEN ATTENTION: ~120–180 wpm caption pacing is within kinetic-typography norms under undivided attention.
- `[risk, per Codex F5 — do not over-claim]` Attention-theft does NOT fully clear the asset: caption dwell + legibility on a ≤36%-height 4:3 monitor is unverifiable off-device → "asset is fine once BIT is silent" stays a hypothesis; a longer-dwell/fewer-fragments recut is the named fallback if the on-device check still reads fast.
- `[validated, grounded]` **Cinema needs lights-down, not a bigger picture:** at 390pt width a full-width 4:3 is already ~34% of screen height (size is ~maxed); what breaks the film feel is full-brightness console chrome (CHARGING header, bar, BIT, keycap) around the picture. `[per Codex F1, high]` any dim must EXCLUDE escape controls — skip/pause/mute keep contrast, placement, and perceived availability (affordance visibility ≠ semantics; low-vision/glare failure path).
- `[validated, grounded]` **The ignition's energy never arrives at the gate (peak-end).** The climax (100% pour, boom, 700ms collapse, 380ms power-on) lands on an admin-first card: "untitled", RECRUIT, EMPTY gray XP bar, 0/50 XP, neutral-pose BIT, ~55% dead dark space. The remembered END of onboarding is bureaucracy. Links the 2026-06-16 peak-end entry (meta-analytic r≈0.58; one clear peak) and the 2026-06-16 "Start Gate stays the sole full reveal" decision — the reveal exists; it just never RECEIVES the charge.
- `[validated, per Codex F3]` **Extra black time is NOT the fix for "transition too fast"** — post-collapse black on Android reads as frozen/lost-tap, not breath. Carry VISIBLE charge continuity across the cut instead (the gate's first beat receives the charge; BIT arrives hyped, continuing the ritual's last line), never dead air.
- `[validated, per Codex F2/F4 — phase invariants are the regression surface]` Skip must stay ONE phase-independent whole-ritual escape (appearance keyed to the held-frame beat, not a raw 3s timer, so it never onsets mid-reel); pressing START BOOSTING must cancel/complete any BIT typewriter atomically (or the clash returns in a new form); route fires once; watchdogs + reduced-motion paths byte-identical.
- **Decision feed:** theater-pass design (BIT silent during playback with intro moved to the self-paced held-frame wait; decorative-only dim; charged-arrival Start Gate reveal; asset recut as measured fallback) → presented for user approval 2026-07-23.

---

## Loading / progress screens — taxonomy, thresholds, honesty (2026-07-23, research skill, Codex needs-attention → all 8 folded)

Decision: define Ironbit's loading-screen doctrine (how many loaders, where, what to adjust). Feeds a follow-up implementation pass on `pixel_loader.dart`, the 3 Material spinners, and `program_loading_page.dart`.

- `[validated, A — NN/g canonical]` **Response-time thresholds are the spine:** 0.1s = instant (no indicator), 1.0s = flow-of-thought preserved, 10s = attention limit (needs determinate % + cancel). Corollary anti-flash rule `[validated, B — practitioner consensus]`: a wait <~300ms must show NO loader (a flash reads as a glitch); either delay-before-show ~100–200ms (skip if the op finishes) or enforce a ~400ms min once shown. → Ironbit's `PixelLoader` currently has neither guard; the exercise-picker/tab loads can flash. ([NN/g response times](https://www.nngroup.com/articles/response-times-3-important-limits/))
- `[validated, A — Buell & Norton, Management Science 2011]` **Labor illusion:** visible effort raises perceived value; people can prefer longer waits *with visible work* to instant results (reciprocity). BOUNDARY `[validated, A/B]`: limits exist — past a tolerance point, and if the OUTCOME is weak, more waiting REDUCES value; obviously-fake/excessive work reads as manipulative (dark-pattern). → the app-open boot + onboarding calibration loaders are legit labor-illusion; the **fixed-5s, NOT-gated-on-real-work program loader is the weak link** — Codex (high): it "trains the user that Ironbit fabricates work before they've experienced product value." Fix: gate on real generation + cap ~2.5–3s + drop the fixed 5s floor; keep the honest input-narration. ([The Labor Illusion](https://www.hbs.edu/faculty/Pages/item.aspx?num=40158))
- `[validated, B — Strong/Hevy teardowns]` **Utility-fitness apps make speed the feature:** local-first, offline logging, auto-populated previous weights, "fastest logging tested," NO loaders in the core loop. → Ironbit's core loop (home, exercise picker, logging, tab switches) should target instant; a spinner there is a smell, not a feature. `[risk, per Codex — do not overclaim]` "instant, no loader" is a GOAL to MEASURE, not a policy: cold start / low-end / large-catalog first-parse can exceed 300ms → keep a loader for genuine >300ms first-loads, kill only the flash. ([Hevy vs Strong](https://setgraph.app/ai-blog/hevy-vs-strong)) ([RepReturn Hevy review](https://repreturn.com/hevy-app-review/))
- `[assumption, B downgraded per Codex — inspiration only]` Diet/AI onboarding "building your plan" loaders (Noom, Cal AI) drive real conversion lift (one A/B: +8.5% trial, +17% paying) — but tied to a PAYWALL. Ironbit has no onboarding paywall, so the business outcome does NOT transfer; borrow only the *psychology* (anticipation + "you built this" endowment/IKEA → identity attachment), and validate Ironbit's onboarding loader with completion/retention, not by analogy. ([Noom teardown](https://www.revenuecat.com/blog/growth/web-to-app-onboarding-funnel/)) ([Cal AI onboarding](https://x.com/cesaralvarezll/status/2036873854455255505))
- `[contested]` **Skeleton vs spinner is NOT a settled win for skeletons.** Practitioner "20–30% faster" is Grade C / single-lineage; a 2017 Viget A/B found skeletons WORST for perceived duration vs spinner/blank. Honest read (per Codex): keep the spinner (`PixelLoader`) for short indeterminate ops, keep skeletons AVAILABLE for known-shape content lists (exercise catalog, history) where previewing structure + preventing layout shift is the real benefit — don't adopt universally, don't ban. ([skeleton-vs-spinner](https://ui-deploy.com/blog/skeleton-screens-vs-spinners-optimizing-perceived-performance))
- `[validated, B — WCAG 2.3.3 + a11y practice]` **Loaders are ESSENTIAL animation** (removing the "working" signal is worse than motion). Reduced-motion best practice: be surgical — no rotation by default; a fully-static icon is acceptable ONLY if paired with status text + a Semantics/live-region announce so it can't read as hung; a subtle non-spatial pulse is optional, not required (per Codex: even pulsing can bother vestibular/distraction-sensitive users). → `PixelLoader` currently ignores reduced motion entirely and has no Semantics label. ([reduced-motion loaders](https://blog.pope.tech/2025/12/08/design-accessible-animation-and-movement/))
- `[validated → doctrine]` **Ironbit loading model = 4 states, not a pile of bespoke loaders:** (0) **inline/state-specific** — button-busy, optimistic-save-with-rollback, empty-vs-loading, error/retry, pull-to-refresh (Codex gap-fill: don't reuse the full-surface spinner for these); (1) **instant / no loader** — core loop, measured, anti-flash; (2) **one functional loader** — the shared `PixelLoader` for genuine >300ms indeterminate in-app waits (unify the 3 stray Material `CircularProgressIndicator`s into it; add reduced-motion + Semantics + anti-flash); (3) **ceremonial/branded** — rare, gated on a genuine transition: app-open boot ✓, onboarding calibration ✓ (gold standard: gated + tap-to-reveal), program build (fix per above). Do NOT add more bespoke loaders.
- `[assumption, per Codex — hypothesis not evidence]` "Loader fatigue" from two multi-second loaders in one onboarding funnel is plausible but unmeasured; the concrete actionable is the fake-5s loader, not a broader cut. Instrument onboarding step-completion / time-to-first-workout / per-loader drop-off before cutting further.

---

## First screen (landing/title) hero + mascot-introduction timing (2026-07-24, research skill, Codex needs-attention → all 4 folded)

Decision: with `app_logo.png` removed from `welcome_landing_page.dart`, what becomes the hero? Contenders: brand/title-only · dormant-BIT vignette · world/CRT standby. BIT is first met DORMANT (cold open, tap-to-wake); face revealed later (Solution).

- `[validated, A — Lindgaard 2006, Behaviour & Information Technology, 1068 cites]` **First impression is formed in ~50ms and HALOS later judgments** of credibility/usability. → the first-screen hero must read attractive/competent/premium *instantly*; anything that reads "off/broken" in the first frame poisons everything after. ([Lindgaard 50ms](https://www.websiteoptimization.com/speed/tweak/blink/))
- `[validated, B — strong practitioner consensus]` **An undesigned off/empty state reads as BROKEN** ("is the app broken? did I do something wrong?"), ~15–25% loss per bad empty-state. → **the risk applies to ANY dark/standby hero, not just a bare BIT core** (Codex F1): a "standby cabinet/CRT" can read as a disabled app just as a bare off-core can. Acceptance bar for any non-brand hero: reads as *intentional, ready/tappable, premium* in ≤50ms. ([empty-state = unfinished](https://medium.com/design-bootcamp/nobody-designs-the-empty-state-thats-exactly-why-your-app-feels-unfinished-da3396570de0))
- `[validated → reframes the user's tension]` **The real hazard is an AWAKE BIT on screen 0 PRE-SPENDING the cold-open wake reveal — not literal awake→asleep "state continuity."** Corollary (Codex F2, the useful flip): **a DORMANT BIT on the first screen RESOLVES the tension** (asleep → asleep → you wake it = continuity), and is a *legitimate contender*, not a rejected one — reject only the *bare/off-core* execution, never dormant-BIT-with-an-intentional-vignette-and-affordance. Preserves the face reveal (BIT stays faceless/asleep) and builds early attachment.
- `[assumption, DOWNGRADED per Codex F3 — do not treat as load-bearing]` "A first screen with zero BIT hint leaves the cold-open wake with no trailer." Anticipation/'movie needs a trailer' logic is drawn from launch/days-apart marketing (Grade B/C) and is **overextended to a one-tap, back-to-back transition**. → **brand/title-only is a SERIOUS primary contender** (cleanest for the 50ms + broken-state + trust risks); the trailer need is a hypothesis to test, not a reason to add a more complex hero.
- `[validated, B — Duolingo teardowns + dark-pattern critique]` Mascot-early builds attachment (Duo appears at/after the welcome, throughout onboarding) — **but Duo is always AWAKE (no dormant→wake arc → not a precedent for a staged reveal), and Duo's guilt/tears turn is widely criticized** and against Ironbit's no-guilt BIT-ethics. Transferable lesson: "a mascot early = attachment," never "an awake/guilt mascot on the title." ([Duo onboarding](https://goodux.appcues.com/blog/duolingo-user-onboarding), [Duo dark patterns](https://opinionsandconditions.substack.com/p/duolingo-owl-dark-patterns-digital-guilt))
- `[validated, B — game title-screen craft]` Two schools: logo-centric (large dominating mark, simple) vs character-focused; **some games deliberately HIDE characters on the title until they appear in-game** — precedent for keeping BIT unseen on the title. ([title screens](https://www.pcgamer.com/our-favorite-title-screens-and-main-menus-in-pc-gaming/))
- `[risk, NEW — Codex F4, load-bearing]` **The fake SIGN IN + beta note COMPOUND a standby/off hero into a trust failure** ("is this app even real/working?"). Blocking first-screen criterion: SIGN IN must be visually **non-primary** / clearly beta-framed (it already is `secondary`), and any standby hero must not imply the *product* is inactive. This weighs against off/standby heroes and toward a confident, clearly-ready first frame.
- **Net (evidence-weighted, no dominant winner — decide by a 50ms mock test, Codex):** three real contenders — **(A) brand/title-only** (safest 50ms/trust, simplest; forfeits early attachment + any tease) · **(B) dormant-BIT vignette with affordance** (resolves continuity, preserves the reveal, early attachment; must pass the "intentional/ready/premium in 50ms" bar and not compound the SIGN-IN doubt) · **(C) world/CRT standby** (character unseen; shares B's broken-state risk, least clearly "a mascot app"). **REJECTED: awake/embodied BIT** (pre-spends the reveal — the user's instinct, corrected) and **a bare off-core** (broken-state). Pick via a first-frame preference/comprehension check across A/B/C.

### Landing composition: why the dormant-BIT landing "feels sad/cramped" (2026-07-24, research, Codex needs-attention → all 3 folded)

Decision: fix the shipped merged-cold-open LANDING (user: "feels terrible, sad · text too cramped to BIT"). Feeds an `ironbit-design` composition pass on `cold_open_page.dart`'s landing state.

- `[validated, A/B — Gestalt proximity, NN/g + Laws of UX]` **CRAMPED CAPTION is the highest-confidence fault** (the user's literal complaint): the tagline jammed on BIT's plates with ~0 gap reads as clutter/ambiguity — "too little differentiation between distances." Fix (low-risk): a deliberate breathing gap from BIT, or regroup the tagline WITH the IRONBIT wordmark as a header (it's currently scattered mid-screen, away from the wordmark). ([NN/g proximity](https://www.nngroup.com/articles/gestalt-proximity/))
- `[validated, B — game idle-animation craft]` **DEAD-PROP STILLNESS** compounds "sad→dead": the dormant BIT is FROZEN (standby ember only, no idle breath) → a lifeless prop. "A ~1px breathing loop is the difference between a living, waiting presence and a static prop." Fix (low-risk within the boundary): a subtle SLEEPING breath. ([idle breathing = alive](https://blog.animschool.edu/2024/06/14/breathing-life-into-idle-animations/))
- `[validated, B — spacing/whitespace UX]` **BROKEN VERTICAL RHYTHM**: unmotivated huge top void + cramped middle + bottom cluster = not purposeful. "Intentional negative space serves hierarchy/rhythm/separation; dead space merely wastes real estate." Fix: grouped header up top · hero centered with balanced breathing room · CTA cluster anchored · consistent scale. ([whitespace guide](https://speckyboy.com/whitespace-hero-area/))
- `[hypothesis, DOWNGRADED per Codex F2 — was the crux, now a hypothesis]` **ISOLATION/negative-space = loneliness** ("a small subject in vast empty space conveys isolation/loneliness; the emptiness becomes part of what you're saying") — evocative, but drawn from PHOTOGRAPHY composition transferred to a UI hero, no measured "how much void = sad" threshold, no first-party test. → do NOT make it load-bearing (don't rush to "make BIT huge / fill the void"); the certain defects are the cramp + dead-stillness. Validate by mock A/B before any hero-scale/void-fill change. ([negative-space emotion](https://fstoppers.com/education/using-positive-and-negative-space-composition-photography-622185))
- `[validated, B — visual hierarchy]` **CTA OUT-COMPETES the dim hero**: the neon GET STARTED is the brightest visual weight → the dim BIT reads as a weak/sad afterthought. BUT the fix (bigger/warmer BIT) is IN TENSION with the dormant constraint → needs the boundary below.
- `[risk, HIGH — Codex F1, the load-bearing gate]` **The DORMANT constraint must be OPERATIONALIZED or any warmer/bigger/breathing BIT spends the cold-open wake reveal.** Acceptance criteria for a landing BIT: **no face** (eyes = the Solution reveal); **motion = a SLEEPING breath only** (tiny ~1px float / slow glow-breathe, slow period — never the awake bob/plate-spread/bright surge); **glow = a dim standby range** (cozier than dead-off, but NOT the awake bright turquoise/amber); **visual weight ≤ ~co-equal with the CTA**, never a surge; **before/after the wake must still read as a genuine state change** ("it woke up"). Block any variant that reads "already awake."
- `[validated → method, Codex F3]` **Fix in SLICES, not one bundled redesign** (mixed-quality evidence + CTA-vs-BIT tension = can't attribute success): **Slice 1 (low-risk, do first):** un-cramp the caption + repair vertical rhythm. **Slice 2 (medium):** add the dormant sleeping-breath + cozier standby (within the boundary). **Slice 3 (validate isolation first):** only then consider BIT scale / space-balance. Each slice keeps "wake reveal still feels unspent" as a success criterion.
- `[validated, B — rule of thirds]` **Follow-up (2026-07-24): the header was "jammed at the top edge" because the camera crane LIFTS it** (a header near the top + an upward crane = pushed to ~3%). Fix: **DECOUPLE the brand header from the crane** (a fixed top-level overlay, not in the craned world) so it sits in the **top third** (~13–18%), BIT the **middle third** (~49%), CTAs the **bottom third** (~78–90%) — the rule-of-thirds vertical stack. Also: title **greened** on the landing (brand pop), lerped green→white as BIT wakes (the wake's white was deliberate — reserves neon for the action + doesn't fight BIT's turquoise up close). ([rule of thirds mobile](https://www.uxdesigninstitute.com/blog/guide-to-the-rule-of-thirds-in-ux/))
